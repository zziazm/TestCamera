	...
	for (int y = 0; y < theheight; y++)
		for (int x = 0; x < thewidth; x++)
		{
			...
			for (int j = -1 * radius; j <= radius; j++)
				...
				{
					sumr += (unsigned char) inbits[redOffsetNew(x+i, y+j, thewidth, theheight)];
					sumg += (unsigned char) inbits[greenOffsetNew(x+i, y+j, thewidth, theheight)];
					sumb += (unsigned char) inbits[blueOffsetNew(x+i, y+j, thewidth, theheight)];
				}
			
			// Assign the outbits
			outbits[redOffsetNew(x, y, thewidth, theheight)] = (unsigned char) (sumr / squared);
			outbits[greenOffsetNew(x, y, thewidth, theheight)] = (unsigned char) (sumg / squared);
			outbits[blueOffsetNew(x, y, thewidth, theheight)] = (unsigned char) (sumb / squared);
			outbits[alphaOffsetNew(x, y, thewidth, theheight)] = (unsigned char) 0xff;
		}
